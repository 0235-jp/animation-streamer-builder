...
1. 「音声準備」パネルでテキストから VOICEVOX エンジン/VOICEVOX API（低速）へ送るか、既存の音声ファイルをアップロードします。生成した音声は自動で解析にかけられ、必要なら「音声生成＆動画作成」でレンダリングまで一気通しできます。
...
- **VOICEVOX API（su-shiki / TTS Quest）**  
  - API URL は内蔵の既定値を使用し、話者 ID とテキストを入力するだけで利用できます。  
